...
def mark_as_read(db, req, user_id):
...
def get_notifications_for_user(db, user_id, offset):
    stmt = (
        select(Notification)
        .where(Notification.user_id == user_id)
        .order_by(Notification.created_at.desc())
        .limit(15)
        .offset(offset) # page number specified by the user.
    )
    result = db.execute(stmt)
