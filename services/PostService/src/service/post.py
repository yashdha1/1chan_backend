...
class PostService: 
    ...
    async def get_post_by_id(self, post_id: str):
        try: 
            # todo : put the spectators in the room to emit the messages: # noqa : E501
    ...
    async def patch_post(self, post_id: UUID, uid: UUID, title: str | None = None, body: str | None = None, image_link: str | None = None, edited_by: str = ""): 
        try:
            res = await self.post_repo.patch_post(post_id, uid, title, body, image_link, edited_by)
    ...
    async def search_posts(self, query) :
        try : 
            ...
            log.error(f"Error searching posts for query: {query}")
            raise HTTPException(status_code=500, detail=f"Internal Server Error - {Exception} .")
